...
do {									  \
  if (yydebug)								  \
    {									  \
      YYFPRINTF (LLV_WARNING, "%s ", Title);					  \
      yy_symbol_print (stderr,						  \
		  Type, Value); \
      YYFPRINTF (LLV_WARNING, "\n");						  \
    }									  \
} while (YYID (0))
...
{
  if (yytype < YYNTOKENS)
    YYFPRINTF (LLV_WARNING, "token %s (", yytname[yytype]);
  else
    YYFPRINTF (LLV_WARNING, "nterm %s (", yytname[yytype]);
...
#endif
{
  YYFPRINTF (LLV_WARNING, "Stack now");
  for (; yybottom <= yytop; yybottom++)
    {
      int yybot = *yybottom;
      YYFPRINTF (LLV_WARNING, " %d", yybot);
    }
  YYFPRINTF (LLV_WARNING, "\n");
}
...
#endif
{
  ...
  YYFPRINTF (LLV_WARNING, "Reducing stack by rule %d (line %lu):\n",
	     yyrule - 1, yylno);
  /* The symbols being reduced.  */
  for (yyi = 0; yyi < yynrhs; yyi++)
    {
      YYFPRINTF (LLV_WARNING, "   $%d = ", yyi + 1);
...
#define YYPOPSTACK(N)   (yyvsp -= (N), yyssp -= (N))
  ...
  YYDPRINTF ((LLV_WARNING, "Starting parse\n"));
...
# endif
      ...
      YYDPRINTF ((LLV_WARNING, "Stack size increased to %lu\n",
  ...
  YYDPRINTF ((LLV_WARNING, "Entering state %d\n", yystate));
...
yybackup:
  ...
  if (yychar == YYEMPTY)
    {
      YYDPRINTF ((LLV_WARNING, "Reading a token: "));
      yychar = YYLEX;
    }
  ...
  if (yychar <= YYEOF)
    {
      ...
      YYDPRINTF ((LLV_WARNING, "Now at end of input.\n"));
    }
  else
    {
      yytoken = YYTRANSLATE (yychar);
...
#line 179 "parse.y"
    {
			...
			if (status < 0)
                        {
                                yyerror("case16:setkeymsg_add return status!=0");
				return -1;
                        }
		}
    break;
...
#line 191 "parse.y"
    {
			...
			if (status < 0)
                        {
                                yyerror("case17:setkeymsg_addr return status!=0");
				return -1;
                        }
		}
    break;
...
#line 210 "parse.y"
    {
			...
			if (setkeymsg_addr(SADB_DELETE, (yyvsp[(5) - (6)].num), (yyvsp[(3) - (6)].res), (yyvsp[(4) - (6)].res), 1) < 0)
                        {
                                yyerror("case18:setkeymsg_addr return status!=0");
				return -1;
                        }
#else /* __linux__ */
...
#line 238 "parse.y"
    {
			...
			if (status < 0)
                        {
                                yyerror("case19:setkeymsg_addr return status!=0");
				return -1;
                        }
		}
    break;
...
#line 466 "parse.y"
    {
			...
			if (p_alg_auth == SADB_X_AALG_TCP_MD5) {
				if ((p_key_auth_len < 1) || 
				    (p_key_auth_len > 80))
                                {
                                    yyerror("case44:p_key_auth_len <1 or p_key_auth_len >80");
				    return -1;
                                }
			} else 
#endif
...
#line 574 "parse.y"
    {
			...
			if (status < 0)
                        {
                                yyerror("case61:setkeymsg_addr return status!=0");
				return -1;
                        }
		}
    break;
...
#line 607 "parse.y"
    {
			...
			if (status < 0)
                        {
                                yyerror("case62:setkeymsg_spdaddr_tag return status!=0");
				return -1;
                        }
		}
    break;
...
#line 620 "parse.y"
    {
			...
			if (status < 0)
                        {
                                yyerror("case63:setkeymsg_spdaddr_tag return status!=0");
				return -1;
                        }
		}
    break;
...
#line 653 "parse.y"
    {
			...
			if (status < 0)
                        {
                                yyerror("case64:setkeymsg_spdaddr_tag return status!=0");
				return -1;
                        }
		}
    break;
...
#line 665 "parse.y"
    {
			...
			if (status < 0)
                        {
                                yyerror("case65:setkeymsg_spdaddr_tag return status!=0");
				return -1;
                        }
		}
    break;
	...
	int dplen;
{
	...
	if (!srcs || !dsts)
        {
                yyerror("setkeymsg_spdaddr invalid src dst ip addr");
		return -1;
        }
	...
	int no_spi;
{
	...
	if (!srcs || !dsts)
        {
                yyerror("setkeymsg_addr invalid src dst ip addr");
		return -1;
        }
	...
	struct addrinfo *dsts;
{
	...
	if (!srcs || !dsts)
        {
                yyerror("setkeymsg_add invalid src dst ip addr");
		return -1;
        }
	/* fix up length afterwards */
	setkeymsg0(msg, type, satype, 0);
