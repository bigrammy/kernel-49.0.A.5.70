...
static void stroke_add_conn(private_stroke_socket_t *this, stroke_msg_t *msg)
{
	...
	DBG2(DBG_CFG, "  enable pcscf restore  = %s", msg->add_conn.pcscf_restore?"yes" : "no");
	DBG2(DBG_CFG, "  enable emergency indication  = %s", msg->add_conn.is_emergency?"yes" : "no");
	DBG2(DBG_CFG, "  enable device_identity  = %s", msg->add_conn.device_identity?"yes" : "no");
