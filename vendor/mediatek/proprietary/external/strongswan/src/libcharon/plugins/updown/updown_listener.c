	...
	bool up)
{
	...
	{	/* Config name: apn_xxx */
		...
		notify_wod(N_DETACH, config->get_name(config) + 4, &this->prop);
	}
	else
	{
		...
		{
			this->prop.backoff_time = ike_sa->get_backoff_time(ike_sa);
		...
		case 0:
			if(ike_sa->get_status_code(ike_sa) == PDN_STATUS_OK && ike_sa->get_pcscf_need_update(ike_sa))
			{
				if(pcscf)
				{
					if(config)
					{
						notify_wod(N_PCSCF_RESTORE, config->get_name(config) + 4, &this->prop);
