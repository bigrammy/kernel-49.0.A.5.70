...
#define GPIO_PIN_IRQ  94 //kent
#define GPIO_PIN_RESET 91 //kent
#if 0
#define GPIO_PIN_33V 0
#endif
...
//	g_data->spi = spi;
    ...
    printk(KERN_ERR "fpc1020_spi_probe exit------\n");
	return error;
}
...
//add for dual sensor end
	...
	if(fps_variant_id() != FPS_EGIS_ID)
	{
		pr_err("[EGIS] fps Device do not exist \n");
		return -ENXIO;
	}
	//[SM32][Kent][17080801][Begin]get fps id pin information
 	...
 	if (status)
 	{
 		printk(KERN_ERR "platform_driver_register%s\n", __func__);
 	        return -EINVAL;
 	}
