...
//#include <soc/qcom/scm.h> //[SM32][kent] remove
...
//kent this will reject the command when estd is service
	...
	struct etspi_data *data)
{
	...
	of_property_read_u32_array(np, "reset-gpio",
						   &gpio, 1);;
	if (gpio < 0) {
		errorno = gpio;
		goto dt_exit;
	...
	//gpio = 94;//of_get_named_gpio(np, "egis,irq-gpio", 0);
	of_property_read_u32_array(np, "irq-gpio",
							   &gpio, 1);
	if (gpio < 0) {
		errorno = gpio;
		goto dt_exit;
...
#else
static int etspi_probe(struct spi_device *pdev);
static int etspi_remove(struct spi_device *pdev);
#endif
...
/////////////////////////////////////////////////////////////////////////////////
//kent
static struct spi_driver etspi_driver = {
	.driver = {
		.name = "et510",
		.bus = &spi_bus_type,
		.owner = THIS_MODULE,
...
static int __init gf_init(void)
{
	...
	status = spi_register_driver(&etspi_driver);
	if (status < 0) {
		DEBUG_PRINT( "%s, Failed to register SPI driver.\n", __func__);
		return -EINVAL;
	}
...
{
	//FUNC_ENTRY();
	spi_unregister_driver(&etspi_driver);
	//FUNC_EXIT();
}
module_exit(gf_exit);
...
///////////////////////////////////////////////////////////
//kent
	status = spi_setup(pdev);
	if(status < 0)
		DEBUG_PRINT("spi_setup failed, ret = %d\n", status);
////////////////////////////////////////////////////////////////////////////////
...
//	DEBUG_PRINT("fp_ioctl >>> fp Trigger function init\n");
//	status = Interrupt_Init(etspi, 0,
//					0,
//					0);
//	DEBUG_PRINT("fp_ioctl trigger init = %x\n", status);

	////////////////////////////////////////////////////////////
	ms = spi_master_get_devdata(etspi->spi->master);
