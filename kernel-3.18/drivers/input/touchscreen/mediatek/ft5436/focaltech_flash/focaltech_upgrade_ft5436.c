...
/*[Arima_8100][allen_yu] Dynamic detect SM31_TP module and SM32_TP module 20170606 begin*/
    ...
    if (fw_valid)
		...
		//ret = fts_i2c_read_reg(client, 0xA0, &vendor_id);
    else
...
#if 1
	FTS_DEBUG("chid_id = 0x%x" , chip_id);
	if (chip_id ==SM31_IC)
	{
		g_fw_file = CTPM_SM31_FW;
		g_fw_len =   fts_getsize(FW_SM31_SIZE);
		FTS_DEBUG("[UPGRADE]FW FILE:SM31_CTPM_FW, SIZE:0x%x", g_fw_len);
	}
	else if (chip_id == SM32_IC)
	{

			//ret = fts_i2c_read_reg(client, FTS_REG_VENDOR_ID, &module_type);
			//FTS_DEBUG("[UPGRADE] module_type=0x%x", module_type);

		if(mt_get_lcm_id_gpio_value(LCM_GPIO)==0)
		{
			g_fw_file = CTPM_SM32_TRULY_FW;
			g_fw_len =  fts_getsize(FW_SM32_TRULY_SIZE);
			FTS_DEBUG("[UPGRADE]FW FILE:SM32_TURLY_CTPM_FW, SIZE:0x%x", g_fw_len);
		}
		else
		{
			g_fw_file = CTPM_SM32_DJN_FW;
			g_fw_len =  fts_getsize(FW_SM32_DJN_SIZE);
			FTS_DEBUG("[UPGRADE]FW FILE:SM32_DJN_CTPM_FW, SIZE:0x%x", g_fw_len);
		}
	}
	else if (chip_id == NO_VALUE)
	{
		...
		else
		{
			...
			/*
			g_fw_file = CTPM_SM32_FW;
    			g_fw_len =  fts_getsize(FW_SM32_SIZE);
    			printk("[UPGRADE]FW FILE:CTPM_FW, SIZE:%x\n", g_fw_len);*/
	}
#else
    			g_fw_file = CTPM_FW;
				g_fw_len = fts_getsize(FW_SIZE);
