...
{
	//printk("Enther tpd_local_init allenyu \n");
    int retval;
    /*set TP volt*/
	...
	printk("regulator_init success allenyu \n");
/*[Arima_8103][Allen_Yu] power-on sequence 20170517 begin*/
#if 0
...
#endif
/*[Arima_8103][Allen_Yu] 20170517 end*/
	printk("power-on success allenyu \n");
    return 0;
}
...
#ifdef ESD_CHECK
#if 0
	printk("finger_num: %d finger, %d line in %s\n",data->touch_point_num,__LINE__,__FUNCTION__);
#endif
    if (data->touch_point_num==0)
  	{
#if 0
	ret_workaround = fts_i2c_read_reg(fts_i2c_client, FTS_GUESTURE_ID, &reg_value);
	printk("reg_value = %d\n" , reg_value);
		if(ret_workaround < 0)
		{
			FTS_ERROR("fts_i2c_read_reg fail : %d line in %s\n",__LINE__,__FUNCTION__);
		}
		if  (reg_value ==0x66)
		{
			FTS_DEBUG("Got Not touch Event interrupt :  %d line in %s\n",__LINE__,__FUNCTION__);
			fts_i2c_write_reg(fts_i2c_client, FTS_GUESTURE_ID, 0x00);
		}
		else
		{
			live_state = 0;
			FTS_ERROR("Touch IC is dead, live_state = 0: %d line in %s\n",__LINE__,__FUNCTION__);
		}
#endif
		return 0;
  	}else{
		if(touch_down_flag==1){
		    ...
		    //printk("disable schedule_delayed_work:  %d line in %s\n",__LINE__,__FUNCTION__);
		}
	}
#endif
/*[Arima_8103][allen_yu] 20170822 end*/
...
/*[Arima_8103][allen_yu] SW ESD Work around 20170822 begin*/
		...
		if(touch_down_flag == 0){
			touch_down_flag = 1;
		    schedule_delayed_work(&check_work, msecs_to_jiffies(4500));
#if 0
			printk("enable schedule_delayed_work:  %d line in %s\n",__LINE__,__FUNCTION__);
#endif
		}
#endif
/*[Arima_8103][allen_yu] 20170822 end*/
...
/*[Arima_8103][bozhi_lin] fix Coverity issues 20170621 begin*/
#if FTS_MT_PROTOCOL_B_EN
    memset(&pevent, 0, sizeof(struct ts_event));
#else
    memset(&cinfo, 0, sizeof(struct touch_info));
...
/*[Arima_8103][allen_yu] SW ESD Work around 20170822 begin*/
	...
	live_state=1;
#endif
/*[Arima_8103][allen_yu] 20180822 end*/
...
static void elan_ktf_ts_check_work_func(struct work_struct *work)
{
	int res = 0;
#if 0
		printk("Enter Workqueue \n");

		disable_irq(private_ts->client->irq);//1102

	flush_work(&A_work);	//1102
#endif
	if (fwupd_flag == 1)
	{
		schedule_delayed_work(&check_work, msecs_to_jiffies(4500));
	...
	if(live_state==1)
	{
		...
		schedule_delayed_work(&check_work, msecs_to_jiffies(4500));
#if 0
		printk("Tp active :  %d line in %s\n",__LINE__,__FUNCTION__);
		enable_irq(private_ts->client->irq);
#endif
		return;
	}

	FTS_ERROR(KERN_EMERG "%s, chip may crash, we need to reset it \n",__FUNCTION__);	

	fts_reset_proc(200);

	FTS_ERROR("Reset TP IC \n");

	fts_irq_disable(); //1102
...
/*[Arima_8103][allen_yu] Show the FW is main source or second source 20171012 begin*/
static int mt_get_lcm_id_gpio_value(int gpio_num)
{
	int value = 0;
...
/*[Arima_8103][allen_yu] 20171012 end*/
...
/*[Arima_8103][Allen_Yu] Adjust sequence of firmware upgrade 20170524 begin*/
    fts_ctpm_upgrade_init();

		fts_i2c_read_reg(client, 0xa6, &ver);

		TPD_DMESG(TPD_DEVICE " fts_i2c_read_reg version : %d\n", ver);

/*[Arima_8103][allen_yu] Show the FW is main source or second source 20171012 begin*/
/*[Arima_8103][Allen_Yu] store touch vendor and firmware version to tpd_show_vendor_firmware 20170518 begin*/
	#if defined(TPD_REPORT_VENDOR_FW)
	{
		extern char *tpd_show_vendor_firmware;
		char buf[80]={0};
		if(mt_get_lcm_id_gpio_value(LCM_GPIO)==0){
			sprintf(buf, "FocalTech_Truly_0x%02X", ver);
		}
		else{
			sprintf(buf, "FocalTech_DJN_0x%02X", ver);
		}
	...
	#endif
/*[Arima_8103][Allen_Yu] 20170518 end*/
/*[Arima_8103][Allen_Yu] 20170524 end*/
/*[Arima_8103][Allen_Yu] 20171012 end*/
#endif
...
/*[Arima_8103][allen_yu] SW ESD Work around 20170822 begin*/
#ifdef ESD_CHECK
	INIT_DELAYED_WORK(&check_work, elan_ktf_ts_check_work_func);	// reset if check hang
	schedule_delayed_work(&check_work, msecs_to_jiffies(4500));
#endif
/*[Arima_8103][allen_yu] 20170822 end*/
    FTS_FUNC_EXIT();

	printk("allenyu_success\n");
...
/*[Arima_8103][Allen_Yu] suspend sequence 20170517 begin*/
	tpd_gpio_output(tpd_rst_gpio_number, 0);
	msleep(1);
/*[Arima_8103][Allen_Yu] 20170517 end*/
#if FTS_POWER_SOURCE_CUST_EN
