...
/*[Arima_8103][allen_yu] Modify dynamic detect SM31_module and SM_32_module 20170719 begin*/
u8 CTPM_SM31_FW[] =
{
#include FTS_UPGRADE_FW2_APP
};
u8 CTPM_SM32_TRULY_FW[] =
{
...
/*[Arima_8103][allen_yu] 20170719 end*/
...
/*[Arima_8103][allen_yu] Dynamic detect main source and second source 20170925 begin*/
/*[Arima_8103][allen_yu] Modify dynamic detect SM31_module and SM_32_module 20170719 begin*/
    else if (fw_type == FW_SM32_TRULY_SIZE)
        fw_len = sizeof(CTPM_SM32_TRULY_FW);
	else if (fw_type == FW_SM32_DJN_SIZE)
        fw_len = sizeof(CTPM_SM32_DJN_FW);
    else if (fw_type == FW_SM31_SIZE)
        fw_len = sizeof(CTPM_SM31_FW);
/*[Arima_8103][allen_yu] 20170719 end*/
/*[Arima_8103][allen_yu] 20170925 end*/
...
static int fts_ctpm_check_fw_ver(struct i2c_client *client)
{
	u8 uc_tp_fm_ver;
	u8 uc_host_fm_ver = 0;
/*[Arima_8103][allen_yu] Dynamic detect SM31_FW and SM32_FW 20170711 begin*/
	u8 fw_id = 0;
/*[Arima_8103][allen_yu] Dynamic detect main source and second source 20170925 begin*/
	u8 SM32_FW_Type = 0;
/*[Arima_8103][allen_yu] 20170925 end*/
...
/*[Arima_8103][allen_yu] Dynamic detect SM31_FW and SM32_FW 20170711 begin*/
	ret = fts_i2c_read_reg(client, 0xA2, &fw_id);
	FTS_DEBUG("[UPGRADE] tp_fw_id=%x", fw_id);
/*[Arima_8103][allen_yu] 20170711 end*/
...
/*[Arima_8103][allen_yu] Dynamic detect SM31_FW and SM32_FW 20170711 begin*/
	if (fw_id ==0x01)
	{
		ret = fts_i2c_read_reg(client, 0xA8, &SM32_FW_Type);
		FTS_DEBUG("[UPGRADE] SM32_module_type=%x", SM32_FW_Type);

		if(SM32_FW_Type ==0x5a && mt_get_lcm_id_gpio_value(LCM_GPIO)==0)
		{
			FTS_DEBUG("[UPGRADE]LCM_ID:%d", mt_get_lcm_id_gpio_value(LCM_GPIO));
			FTS_DEBUG("Host FW is Turly, and module is also turly");
		...
		else if(SM32_FW_Type ==0x5a && mt_get_lcm_id_gpio_value(LCM_GPIO)==1)
		{
			FTS_DEBUG("[UPGRADE]LCM_ID:%d", mt_get_lcm_id_gpio_value(LCM_GPIO));
			FTS_DEBUG("Host FW is Turly, but module is DJN");
			return 1;
		}
		else if(SM32_FW_Type ==0x67 && mt_get_lcm_id_gpio_value(LCM_GPIO)==1)
		{
			FTS_DEBUG("[UPGRADE]LCM_ID:%d", mt_get_lcm_id_gpio_value(LCM_GPIO));
			FTS_DEBUG("Host FW is DJN, and module is also DJN");
			if (uc_tp_fm_ver < uc_host_fm_ver )
			{
				return 1;
			}
		...
		{
			FTS_DEBUG("[UPGRADE]LCM_ID:%d", mt_get_lcm_id_gpio_value(LCM_GPIO));
			FTS_DEBUG("Host FW is DJN, but module is Turly");
			return 1;
		}
	}
	else
	{
		FTS_DEBUG("Host FW is SM31_FW");
		return 1;
	}
/*[Arima_8103][allen_yu] 20170711 end*/
/*[Arima_8103][allen_yu] 20170925 end*/
...
/*[Arima_8100][allen_yu] Modify dynamic detect SM31_module and SM_32_module 20170719 begin*/
	...
	/* Dynamic Focaltech Touch IC */
	i_ret = fts_i2c_read_reg(client, 0xA0, &ic_type_id);
	FTS_DEBUG("[UPGRADE] ic_type_id=0x%x", ic_type_id);



	if (i_ret < 0)    
		{        
		FTS_ERROR("Get ic type fail");        
		return 0;    
		}
	...
	case SM31_IC_TYPE :
		...
		//memset(CTPM_FW, 0, CTPM_SM31_FW); 
		//memcpy (CTPM_FW, CTPM_SM31_FW, sizeof(CTPM_SM31_FW));
		break;
	case SM32_IC_TYPE :
		...
		//memset(CTPM_FW, 0, CTPM_SM32_FW); 
		//memcpy (CTPM_FW, CTPM_SM32_FW, sizeof(CTPM_SM32_FW));
		break;
...
/*[Arima_8103][Allen_Yu] store touch vendor and firmware version to tpd_show_vendor_firmware 20170518 begin*/
	#if defined(TPD_REPORT_VENDOR_FW)
	{
		...
		fts_i2c_read_reg(fts_i2c_client, 0xA6, &ver);
