...
/*[Arima_8103][allen_yu] FT5436 add Noise Test and Uniformity test to AATS test 20170712 begin*/
#define REG_FW_PROCESS  0x1A
#define REG_RELEASECODEID_L  0xAF
#define REG_RELEASECODEID_H  0xAE
/*[Arima_8103][allen_yu] 20170712 end*/
...
static int m_TempNoiseData[TX_NUM_MAX][RX_NUM_MAX] = {{0}};
static int m_AvgData[TX_NUM_MAX][RX_NUM_MAX] = {{0}};
static int iRawDataAvr[TX_NUM_MAX][RX_NUM_MAX] = {{0}};
static int m_iTempData[TX_NUM_MAX][RX_NUM_MAX] = {{0}};
...
unsigned char FT5X46_TestItem_NoiseTest(bool * bTestResult);      //add by LiuWeiGuang
/*[Arima_8103][allen_yu] 20170712 end*/
...
/*[Arima_8103][allen_yu] FT5436 add Noise Test and Uniformity test to AATS test 20170712 begin*/		
        ...
        if (Code_FT5X46_NOISE_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
           )
        {

            ReCode = FT5X46_TestItem_NoiseTest(&bTempResult);
            if (ERROR_CODE_OK != ReCode || (!bTempResult))
            {
                bTestResult = false;
                g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
...
unsigned char FT5X46_TestItem_RawDataTest(bool * bTestResult)
{
    ...
    int RawDataMax;
    unsigned char ucFre;
/*[Arima_8103][allen_yu] Modify the return value of Noise test 20170929 begin*/
    unsigned char ucFir;
/*[Arima_8103][allen_yu] 20170929 end*/
...
/*[Arima_8103][allen_yu] Modify the return value of Noise test 20170929 begin*/
    ReCode = WriteReg( 0x0A, ucFre );//set the origin value
    if (ReCode != ERROR_CODE_OK) {
        FTS_TEST_ERROR("\n\n Write 0x0A error. Error Code: %d",  ReCode);
        goto TEST_ERR;
    }
    ReCode = WriteReg( 0xFB, ucFir );//set the origin value
    if (ReCode != ERROR_CODE_OK) {
...
/*[Arima_8103][allen_yu] FT5436 add Noise Test and Uniformity test to AATS test 20170712 begin*/
        Save_Test_Data(m_RawData, g_ScreenSetParam.iTxNum+0, 0, 2, g_ScreenSetParam.iRxNum, 1);
/*[Arima_8103][allen_yu] 20170712 end*/
    }
...
//       ibiggerValue = g_ScreenSetParam.iTxNum>g_ScreenSetParam.iRxNum?g_ScreenSetParam.iTxNum:g_ScreenSetParam.iRxNum;
/*[Arima_8103][allen_yu] FT5436 add Noise Test and Uniformity test to AATS test 20170712 begin*/
        Save_Test_Data(m_RawData, g_ScreenSetParam.iTxNum+2, 0, 2, g_ScreenSetParam.iRxNum, 2);
/*[Arima_8103][allen_yu] 20170712 end*/
    }
    //-----4. post-stage work
    if (m_bV3TP)
...
unsigned char FT5X46_TestItem_UniformityTest(bool * bTestResult)
{
    ...
    {
        *bTestResult = true;
        FTS_TEST_INFO("Uniformity Test is OK.");
    }
    else
    {
        *bTestResult = false;
        FTS_TEST_INFO("Uniformity Test is NG.");
    }
...
unsigned char FT5X46_TestItem_NoiseTest(bool * bTestResult)
{
    unsigned char ReCode = ERROR_CODE_OK;
    bool btmpresult = true;
    unsigned char chValue = 0;
    bool bFWProcess=false;
    unsigned char chFir = 0;
    unsigned char chNewFir=0;
    int iNoiseFrames=0;
    int iNoiseTime=0;
...
/*[Arima_8103][allen_yu] 20170929 end*/
    ...
    //如果?取1a寄存器的值?1，且?有??使用工具??，?使用FW中?理的noise?据
    ...
    if ((false == g_stCfg_FT5X46_BasicThreshold.bNoiseTest_TPToolProcess) && (1 == strSwitch))
    {
        bFWProcess = true;
    }


    if (g_stCfg_FT5X46_BasicThreshold.bNoiseTest_GloveMode)
    {
        FTS_TEST_INFO("\r\n\r\n==============================Test Item: -------- Noise Test In Glove Mode\r\n\r\n");

        //?寄存器，?入手套模式
        ReCode = (( ReCode == ERROR_CODE_OK ) &&  WriteReg( 0xF6, 0x01 ));
        SysDelay( 10 );
        if ( ReCode != ERROR_CODE_OK )
        {
    ...
    //?FW中?取diff?据;0x06:1?differ?据；0X06:0?rawdata?据
    //ReCode = theDevice.m_cHidDev[m_NumDevice]->ReadReg( 0x06, &chOldValue );
    ...
    //先判?是否?v3屏体，然后?取0x54的值，并判?与?定的mapping?型是否一致，不一致?入?据
    //noise mapping后，mapping前：0x54=1;mapping后：0x54=0;
    if (m_bV3TP)
    {
        ReCode = ReadReg( REG_MAPPING_SWITCH, &strSwitch );
        if (strSwitch != 0)
    ...
    //?置?率???的?率
    chNewFir = g_stCfg_FT5X46_BasicThreshold.Set_Frequency;
    ReCode =WriteReg( 0x0A,chNewFir/*0x00*/);
    SysDelay( 20 );
    if ( ReCode != ERROR_CODE_OK )
    {
        FTS_TEST_ERROR("\r\nFailed to Write Reg!\r\n ");
    ...
    //如果切?了??，?始采集的?据?有很大差异，故而需要舍?一些?据
    if (0x00 != chNewFir)
    {
        for ( i=0; i<30; i++)
        {
/*[Arima_8103][allen_yu] fix Coverity issues 20170721 begin*/
            ReCode = StartScan();
            SysDelay( 20 );
        }
		if (ReCode != ERROR_CODE_OK)
        {
            FTS_TEST_ERROR("[%s]StartScan Failed!", __func__);
        }
    }
/*[Arima_8103][allen_yu] 20170721 end*/	
    //////////////////////////////////////////////////////////求Noise -Start

    iNoiseFrames = g_stCfg_FT5X46_BasicThreshold.NoiseTest_Frames;
    iNoiseTime = g_stCfg_FT5X46_BasicThreshold.NoiseTest_Time;
    bSampModeFrame = g_stCfg_FT5X46_BasicThreshold.NoiseTest_SampeMode == 0;
    bNoiseTip = g_stCfg_FT5X46_BasicThreshold.NoiseTest_ShowTip == 1;

    memset(m_NoiseData, 0, sizeof(m_NoiseData));

    if (bFWProcess)
    {
        //?OX1C?入ini?置的??;?0X1B?1后?取FW?理后的noise?据
    ...
    else
    {
        ...
        if (NT_AvgData == g_stCfg_FT5X46_BasicThreshold.NoiseTest_NoiseMode)
        {
            ...
            for (i = 0; i < iNoiseFrames; i++)
            {
                for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++)
                {
                    for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)
                    {
                        iValue = focal_abs(m_TempNoiseData[i][iRow*g_ScreenSetParam.iRxNum + iCol] - iRawDataAvr[i][j]);
        ...
        else if (NT_MaxData == g_stCfg_FT5X46_BasicThreshold.NoiseTest_NoiseMode)
        {
        ...
        else if (NT_MaxDevication == g_stCfg_FT5X46_BasicThreshold.NoiseTest_NoiseMode)
        {
/*[Arima_8103][allen_yu] fix Coverity issues 20170809 begin*/
            //CaculateNoiseBaseOnMaxMin(iRawDataAvr, iNoiseFrames);
            memset(m_iTempData, 0xffffffff, sizeof(m_iTempData));//Save The Min Value
/*[Arima_8103][allen_yu] 20170809 end*/
        ...
        else if (NT_DifferData == g_stCfg_FT5X46_BasicThreshold.NoiseTest_NoiseMode)
        {
            //Caculate the Avg Value of all nodes
            memset(m_NoiseData, 0, sizeof(m_NoiseData));
            for (i = 0; i< iNoiseFrames; i++)
    ...
    if (!g_stCfg_FT5X46_BasicThreshold.bNoiseThreshold_Choose)
    {
        if (g_stCfg_FT5X46_BasicThreshold.bNoiseTest_GloveMode)
        {
            //read Reg:0XF7
            ...
            //目前?有手套模式了
            //iNoiseMax = (g_stCfg_FT5X46_BasicThreshold.GloveNoiseTest_Coefficient/100)*chNoiseValue;
    ...
    for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++)
    {
        for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)
        {
            if (!g_stCfg_FT5X46_BasicThreshold.bNoiseThreshold_Choose)
            {
                ...
                {
                    maxHole[iRow][iCol] =g_stCfg_MCap_DetailThreshold.NoistTest_Coefficient[iRow][iCol]*chNoiseValue*4/100;
                }
            }
            else
            {
                maxHole[iRow][iCol] = g_stCfg_FT5X46_BasicThreshold.NoiseTest_Threshold;
            }
        }
    }

    if ( 1 == g_stCfg_FT5X46_BasicThreshold.NoiseTest_NoiseMode && g_stCfg_FT5X46_BasicThreshold.NoiseTest_MinNgFrame && !bFWProcess)
    {
        iMinNGTime = g_stCfg_FT5X46_BasicThreshold.NoiseTest_MinNgFrame;
        iMinNGTime = max( 1, iMinNGTime );
        iMinNGTime = min( iNoiseFrames , iMinNGTime );
        for (i = 0; i < iNoiseFrames && iFrameHasNG; i++)
        {
            for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++)
            {
                for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)
                {
                    m_NoiseDataTemp[iRow][iCol]  = focal_abs (m_TempNoiseData[i][iRow*g_ScreenSetParam.iRxNum + iCol]);

                    if ( (0 == g_stCfg_MCap_DetailThreshold.InvalidNode[iRow][iCol]) )
                    {
                        continue;
                    }
                    if (m_NoiseDataTemp[iRow][iCol] <  minHole[iRow][iCol]  || m_NoiseDataTemp[iRow][iCol] >  maxHole[iRow][iCol] )
                    {
/*[Arima_8103][allen_yu] Modify the return value of Noise test 20170929 begin*/
                        /*bResult*/btmpresult = false;
/*[Arima_8103][allen_yu] 20170929 end*/
                        FTS_TEST_ERROR("noise test failure. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d)  ",  \
                                     iRow+1, iCol+1, m_NoiseDataTemp[iRow][iCol], minHole[iRow][iCol], maxHole[iRow][iCol]);
                    }
        ...
        if ( iFrameHasNG >= iMinNGTime )
            memcpy( m_NoiseData, m_NoiseDataTemp, sizeof(m_NoiseDataTemp) );
        else
        {
            memset( m_NoiseData, 0, sizeof(m_NoiseData) );
            for (index = 0; index < iNoiseFrames; ++index )
            {
                for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++)
                {
                    for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)
                    {
                        m_NoiseData[iRow][iCol] = max( m_NoiseData[iRow][iCol], focal_abs (m_TempNoiseData[i][iRow*g_ScreenSetParam.iRxNum + iCol]));
                    }
                }
            }
...
/*[Arima_8103][allen_yu] 20170929 end*/
    ...
    {
        FTS_TEST_DBG("\r\nNoise:Original Value: %d, Max: %02d, Min: %02d, Avg: %02d ", chNoiseValue, maxValue, minValue,AvgValue);
    }


    ////<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<收集???据，存入CSV文件

    //在FIR=1，默??率下,?Rawdata_min大于等于?值??PASS，小于??值??NG。 范?3000~15000
    if (g_stCfg_FT5X46_BasicThreshold.bNoiseTest_GloveMode)
    {
        //?率?默??率
...
/*[Arima_8103][allen_yu] fix Coverity issues 20170721 begin*/
			...
			if (ReCode != ERROR_CODE_OK)
			{
				FTS_TEST_ERROR("[%s]GetRawData Failed!", __func__);
			}
/*[Arima_8103][allen_yu] 20170721 end*/
        }
        for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++)
        ...
        FTS_TEST_DBG("\r\n\r\nGet Min_Rawdata is:%d; Set Min_Rawdata is %d.\r\n",iRawdataMin,g_stCfg_FT5X46_BasicThreshold.NoiseTest_RawdataMin);

        if (iRawdataMin < g_stCfg_FT5X46_BasicThreshold.NoiseTest_RawdataMin)
        {
            btmpresult=false;
    ...
    //?FW恢复?取Rawdata?据
    ReCode = (ReCode || WriteReg( 0x06,0x00 ));
    SysDelay(200);
    if ( ReCode != ERROR_CODE_OK)
    {
        FTS_TEST_ERROR("\r\nFailed to Read or Write Reg!\r\n ");
...
unsigned char FT5X46_TestItem_WeakShortTest( bool* bTestResult )
{
    ...
    int iMaxTx = 35;
/*[Arima_8103][bozhi_lin] fix Coverity issues 20170621 begin*/
    unsigned char iTxNum = 0, iRxNum = 0, iChannelNum;
/*[Arima_8103][bozhi_lin] 20170621 end*/
...
unsigned char GetTxSC_CB(unsigned char index, unsigned char *pcbValue)
{
    ...
    if (index<128) //single read
    {
        *pcbValue = 0;
		...
		if (ReCode != ERROR_CODE_OK)
        {
            FTS_TEST_ERROR("[%s]WriteReg Failed!", __func__);
        }
        ReCode = ReadReg(REG_ScCbBuf0, pcbValue);
		if (ReCode != ERROR_CODE_OK)
        {
            FTS_TEST_ERROR("[%s]ReadReg Failed!", __func__);
        }
    }
    else//Sequential Read length index-128
    {
		...
		if (ReCode != ERROR_CODE_OK)
        {
            FTS_TEST_ERROR("[%s]Comm_Base_IIC_IO Failed!", __func__);
        }
...
/*[Arima_8100][allen_yu] 20170712 end*/
