...
/*[Arima_8100][allen_yu] set FocalTech ft5436 touch clock rate to 400KHz 20170713 begin*/
		client->timing = 400;
/*[Arima_8100][allen_yu] 20170713 end*/
        ret = i2c_master_send(client, (unsigned char *)g_dma_buff_pa, writelen);
        if (ret != writelen)
        {
            FTS_ERROR("[IIC]: i2c_master_send faild(read), ret=%d!!", ret);
        }
        client->addr = client->addr & I2C_MASK_FLAG &(~ I2C_DMA_FLAG);
    }
...
/*[Arima_8100][allen_yu] 20170713 end*/
		...
		if (ret != writelen)
        {
            FTS_ERROR("[IIC]: i2c_master_send faild(write), ret=%d!!", ret);
        }
        client->addr = client->addr & I2C_MASK_FLAG &(~ I2C_DMA_FLAG);
    }
    mutex_unlock(&i2c_rw_access);
