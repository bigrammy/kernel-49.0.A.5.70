...
#include <linux/clk.h>
//<--[SM31][Sensors][JasonHsing] Hall sensor driver porting 20161004 BEGIN --
#include <linux/input.h>
#include <linux/interrupt.h>
#include <linux/gpio.h>
#include <linux/spinlock.h>
//-->[SM31][Sensors][JasonHsing] Hall sensor driver porting 20161004 END --
...
//<--[SM32][Sensors][JasonHsing] Add node to disable hall sensor 20170622 BEGIN --
static bool hall_enable_flag = true;
//-->[SM32][Sensors][JasonHsing] Add node to disable hall sensor 20170622 END --
#endif
//-->[SM31][Sensors][JasonHsing] Hall sensor driver porting 20161004 END --
/* for keymap handling */
...
//<--[SM32][Sensors][JasonHsing] Add node to disable hall sensor 20170622 BEGIN --
#if HALL_SENSOR_ENABLE
static ssize_t hall_store_enable(struct device_driver *ddri, const char *buf, size_t count)
{
	int hall_enable=1;

	sscanf(buf, "%d",&hall_enable);
	hall_enable_flag = hall_enable;

	if(hall_enable_flag == 0){
...
static ssize_t hall_show_enable(struct device_driver *ddri, char *buf)
{
	int res;
...
#if HALL_SENSOR_ENABLE
static DRIVER_ATTR(hall_enable, S_IWUSR | S_IRUGO, hall_show_enable, hall_store_enable);
#endif
...
//<--[SM31][Sensors][JasonHsing] Hall sensor driver porting 20161004 BEGIN --
	...
	if(hall_enable_flag){
		...
		printk("[hall_sensor] LID_OPEN. lid_state==%d\n",lid_state);
	}
#endif
//-->[SM31][Sensors][JasonHsing] Hall sensor driver porting 20161004 END --
