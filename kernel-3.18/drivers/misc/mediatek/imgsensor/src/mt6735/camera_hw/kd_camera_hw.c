...
//[SM32][Camera]Add Main Camera OV13855 Driver PersonLiu 20170703 S
		if ((currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV13855_MIPI_RAW, currSensorName))) ||
				(currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV13855B_MIPI_RAW, currSensorName))) ||
				(currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K3L8_MIPI_RAW, currSensorName))) ||
...
//[SM32][Camera]Add Main Camera OV13855 Driver PersonLiu 20170703 E
...
// [SM32][Camera][akenhsu] Porting front camera img sensor: ov8856 20170511 BEGIN
		} else  if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8856_MIPI_RAW, currSensorName))) {
			PK_DBG("OV8856 power ON!\n");
			/* VCAM_A */
			if (TRUE != _hwPowerOn(VCAMA, VOL_2800)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable analog power (VCAM_A),power id = %d\n", VCAMA);
				goto _kdCISModulePowerOn_exit_;
...
// [SM32][Camera][akenhsu] 20170511 END
// [SM32][Camera][akenhsu] Add front camera 2nd Source ov8857 20170707 BEGIN
		} else  if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8857_MIPI_RAW, currSensorName))) {
			PK_DBG("OV8857 power ON!\n");
			/* VCAM_A */
			if (TRUE != _hwPowerOn(VCAMA, VOL_2800)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable analog power (VCAM_A),power id = %d\n", VCAMA);
				goto _kdCISModulePowerOn_exit_;
...
// [SM32][Camera][akenhsu] 20170707 END
		} else {
			/* First Power Pin low and Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
...
//[SM32][Camera][akenhsu] Add front camera 2nd Source ov8857 20170707 BEGIN
			...
			(0 == strcmp(SENSOR_DRVNAME_OV8857_MIPI_RAW, currSensorName))) {
			PK_DBG("OV8857 power OFF!\n");
