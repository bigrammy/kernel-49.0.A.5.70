...
kal_uint16 ov8856_read_sensor(kal_uint32 addr)
{
...
void write_ov8856_sensor_lsc(kal_uint32 addr, kal_uint16 lsc[240])
{
...
void ov8856_write_sensor(kal_uint32 addr, kal_uint32 para)
{
...
*
*************************************************************************/
#define OV8856_MODULE_ID	0x02
#define OV885X_READ_ID		0x885a
extern kal_uint16 ov8856_read_module_id(void);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			{
				kal_uint16 read_module_id = ov8856_read_module_id();
				LOG_INF("ReadSensorID = 0x%x, ModuleID=0x%x", *sensor_id, read_module_id);
				if (*sensor_id == OV885X_READ_ID && read_module_id == OV8856_MODULE_ID) {
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)); // +0 is for main source OV8856
			if (sensor_id == imgsensor_info.sensor_id) {				
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x", imgsensor.i2c_write_id,sensor_id);
				break;
			}	
