...
#define MAX_OTP_SIZE 1900
static int ov13855_eeprom_read;
...
union OTP_DATA ov13855_eeprom_data = {{0} };
...
static kal_uint16 read_cmos_sensor_ov(kal_uint16 slave_id, kal_uint32 addr)
{
...
int read_ov13855_eeprom(u8 slv_id, u16 offset, u8 *data)
{
	int ret = 0;

	*data  = read_cmos_sensor_ov(slv_id, offset);
	CAM_CALDB("OTP read slv_id 0x%x offset 0x%x  data 0x%x\n", slv_id, offset, *data);

	return ret;
}

int read_ov13855_eeprom_size(u8 slv_id, u16 offset, u8 *data, int size)
{
	int i = 0;

	for (i = 0; i < size; i++) {
		if (read_ov13855_eeprom(slv_id, offset + i, data + i) != 0)
			return -1;
	}
	return 0;
}
...
int read_ov13855_eeprom_mtk_fmt(void)
{
//	int i = 0;
//	int offset = 0;

	CAM_CALINF("OTP readed =%d\n", ov13855_eeprom_read);
        //[SM32][Camera]Modify for Camera 2nd source Person Liu 20170703 S
	if (1 == ov13855_eeprom_read) {
		CAM_CALDB("OTP readed ! skip Module = 0x%x\n",ov13855_eeprom_data.Data[0x04]);
		return ov13855_eeprom_data.Data[0x04];
	}
        //[SM32][Camera]Modify for Camera 2nd source Person Liu 20170703 E
	spin_lock(&g_CAM_CALLock);
	ov13855_eeprom_read = 1;
	spin_unlock(&g_CAM_CALLock);

	/* read flag */

	read_ov13855_eeprom_size(0xA2, 0x00, &ov13855_eeprom_data.Data[0x00], 1);

	/* Read Module Information */
	read_ov13855_eeprom_size(0xA2, 0x01, &ov13855_eeprom_data.Data[0x01], 8);

	/* Read Mirror Flip */

	read_ov13855_eeprom_size(0xA2, 0x09, &ov13855_eeprom_data.Data[0x09], 1);

	/* read calibration version 0xff000b01 */
	if (read_ov13855_eeprom_size(0xA2, 0x000A, &ov13855_eeprom_data.Data[0x0A], 4) != 0) {
		CAM_CALERR("read ov13855_eeprom M24C64S i2c fail !?\n");
		return -1;
	}

	/* read serial number */
	read_ov13855_eeprom_size(0xA2, 0x0E, &ov13855_eeprom_data.Data[0x0E], 2);

	/* read AF config */
	read_ov13855_eeprom_size(0xA2, 0x11, &ov13855_eeprom_data.Data[0x11], 1);

	/* read AWB */
	read_ov13855_eeprom_size(0xA2, 0x12, &ov13855_eeprom_data.Data[0x12], 8);

	/* read AF calibration */
	read_ov13855_eeprom_size(0xA2, 0x1A, &ov13855_eeprom_data.Data[0x01A], 4);

	/* read LSC size */
	read_ov13855_eeprom_size(0xA2, 0x1E, &ov13855_eeprom_data.Data[0x01E], 2);


	/* for lsc data */
	read_ov13855_eeprom_size(0xA2, 0x20, &ov13855_eeprom_data.Data[0x020],0x74c);
        //[SM32][Camera]Modify for Camera 2nd source Person Liu 20170703 S
	return ov13855_eeprom_data.Data[0x04];
        //[SM32][Camera]Modify for Camera 2nd source Person Liu 20170703 E
}
...
static long ov13855eeprom_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	long ret;
	...
	CAM_CALDB("[CAMERA SENSOR] ov13855_eeprom_DEVICE_ID,%p %p %x ioc size %d\n",
...
{
	memcpy((void *)data, (void *)&ov13855_eeprom_data.Data[offset], size);
...
*/
	...
	case CAM_CALIOC_G_READ:
		...
		OV13855_EEPROM_DEVICE_ID, ptempbuf->u4Length);
...
static const struct file_operations g_stCAM_CAL_fops = {
	...
	/* .ioctl = CAM_CAL_Ioctl */
#ifdef CONFIG_COMPAT
	.compat_ioctl = ov13855eeprom_Ioctl_Compat,
#endif
...
static int __init CAM_CAL_init(void)
{
	...
	if (platform_driver_register(&g_stCAM_CAL_Driver)) {
		CAM_CALERR("failed to register ov13855_eeprom driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stCAM_CAL_Device)) {
		CAM_CALERR("failed to register ov13855_eeprom driver, 2nd time\n");
		return -ENODEV;
	}
