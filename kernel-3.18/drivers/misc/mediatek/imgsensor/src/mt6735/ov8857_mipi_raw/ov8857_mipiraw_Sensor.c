...
static imgsensor_info_struct imgsensor_info = { 
	...
	.hs_video = { // SENSOR_SCENARIO_ID_SLIM_VIDEO1 for FOV_80
		.pclk = 144000000,				//record different mode's pclk
		.linelength  = 1932,			//record different mode's linelength
		.framelength = 2482,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width  = 2128,		//record different mode's width of grabwindow
		.grabwindow_height = 1600,		//record different mode's height of grabwindow
		.mipi_data_lp2hs_settle_dc = 85,
		.max_framerate = 300,
	},
	.slim_video = { // SENSOR_SCENARIO_ID_SLIM_VIDEO2 for FOV_80
		.pclk = 144000000,				//record different mode's pclk
		.linelength  = 1932,			//record different mode's linelength
		.framelength = 2482,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =
{
 ...
 { 3296, 2480,	584,   440, 2128, 1600, 2128, 1600,   4,	2, 2128, 1600,	 0, 0, 2128, 1600}, // hight speed video
 { 3296, 2480,  584,   440, 2128, 1600, 2128, 1600,   4,	2, 2128, 1600,	 0, 0, 2128, 1600}, // slim video for FOV_80
};
...
// Read sensor for OTP code
kal_uint16 ov8857_read_sensor(kal_uint32 addr)
{
	return read_cmos_sensor(addr);
}

#define REG_ACCESS_SIZE	6 // Max trasfer size is 6+2
// for OTP write function
void write_ov8857_sensor_lsc(kal_uint32 addr, kal_uint16 lsc[240])
{
	kal_uint32 reg_addr = 0;
	int i = 0;
	char send_cmd[2 + REG_ACCESS_SIZE] = { 0 };
	for (i = 0; i < 240 / REG_ACCESS_SIZE; i++) {
		reg_addr = addr + i * REG_ACCESS_SIZE;
		send_cmd[0] = (char)(reg_addr >> 8);
		send_cmd[1] = (char)(reg_addr & 0xFF);
		send_cmd[2] = lsc[i * REG_ACCESS_SIZE + 0];
		send_cmd[3] = lsc[i * REG_ACCESS_SIZE + 1];
		send_cmd[4] = lsc[i * REG_ACCESS_SIZE + 2];
		send_cmd[5] = lsc[i * REG_ACCESS_SIZE + 3];
		send_cmd[6] = lsc[i * REG_ACCESS_SIZE + 4];
		send_cmd[7] = lsc[i * REG_ACCESS_SIZE + 5];
		iWriteRegI2C(send_cmd, 2 + REG_ACCESS_SIZE, imgsensor.i2c_write_id);
	}
}
...
// Write sensor for OTP code
void ov8857_write_sensor(kal_uint32 addr, kal_uint32 para)
{
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
	...
	LOG_INF("framerate=%d, min_framelength_en=%d ", framerate,min_framelength_en);
...
static void set_shutter(kal_uint16 shutter)
{
	...
	LOG_INF("Exit! shutter =%d, framelength =%d", shutter,imgsensor.frame_length);
...
{
	LOG_INF("le:0x%x, se:0x%x, gain:0x%x",le,se,gain);
...
static void sensor_init(void)
{
    ...
    */
	...
	{
		// Initial OTP setting
		extern void ov8857_otp_update(void);
		ov8857_otp_update();
	}
}	/*	sensor_init  */
...
static void hs_video_setting(void) // For FOV_80
{
	LOG_INF("E");
	write_cmos_sensor(0x0103, 0x01);
	write_cmos_sensor(0x0100, 0x00);
...
//	write_cmos_sensor(0x5000, 0x57);
	...
	// key setting for 2128x1600 cropping from 3264x2448
	write_cmos_sensor(0x3808, 0x08);
	write_cmos_sensor(0x3809, 0x50);
...
static void slim_video_setting(void) // For FOV_80
{
	LOG_INF("E");
	write_cmos_sensor(0x0103, 0x01);
	write_cmos_sensor(0x0100, 0x00);
...
*
*************************************************************************/
#define OV8857_MODULE_ID	0x42
#define OV885X_READ_ID		0x885a
extern kal_uint16 ov8857_read_module_id(void);
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
				}
			}	
			{
				kal_uint16 read_module_id = ov8857_read_module_id();
				LOG_INF("ReadSensorID = 0x%x, ModuleID=0x%x", *sensor_id, read_module_id);
				if (*sensor_id == OV885X_READ_ID && read_module_id == OV8857_MODULE_ID) {
					*sensor_id = OV8857_SENSOR_ID;
					break;
				} else {
					*sensor_id = 0xFFFFFFFF;
				}
			}
			LOG_INF("Read sensor id fail, id: 0x%x - 0x%x", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)) + 1; // +1 is for second source OV8857
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x", imgsensor.i2c_write_id,sensor_id);
				break;
			}	
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {//15fps
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			LOG_INF("Warning: current_fps %d fps is not support, so use cap1's setting: ?? fps!",imgsensor_info.cap1.max_framerate/10);
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate) 
{
	...
	switch (scenario_id) {
		...
		default:  //coding with  preview scenario by default
			...
			spin_unlock(&imgsensor_drv_lock);
			set_dummy();	
			LOG_INF("error scenario_id = %d, we use preview scenario ", scenario_id);
			break;
	}	
	return ERROR_NONE;
}
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_SET_FRAMERATE:
            LOG_INF("current fps :%d", (UINT32)*feature_data);
            spin_lock(&imgsensor_drv_lock);
            imgsensor.current_fps = *feature_data;
            spin_unlock(&imgsensor_drv_lock);
            break;
        case SENSOR_FEATURE_SET_HDR:
            LOG_INF("ihdr enable :%d", (BOOL)*feature_data);
            spin_lock(&imgsensor_drv_lock);
            imgsensor.ihdr_en = (BOOL)*feature_data;
            spin_unlock(&imgsensor_drv_lock);
            break;
        case SENSOR_FEATURE_GET_CROP_INFO:
            LOG_INF("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d", (UINT32)*feature_data);
        ...
        case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
            LOG_INF("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d",(UINT16)*feature_data,(UINT16)*(feature_data+1),(UINT16)*(feature_data+2));
