...
/*
 * check there exists external RF buck or not
 * Returned value
 *   0:no external RF buck
 *	 1:got external RF buck
 */
int is_ext_RF_buck_exist(void)
{
	static int first_check = 1;
	static int got_ext_rf_buck = 1;

	if (first_check) {
		first_check = 0;
		got_ext_rf_buck = mt_get_gpio_in(GPIO81);
		if (got_ext_rf_buck)
			pr_err("Got external RF Buck\n");
		else
			pr_err("No external RF Buck\n");
	}

	return got_ext_rf_buck;
}
