...
//<--[SM32][Sensors][JasonHsing] Add ALS/PS parameter tuning version_2 20170703 BEGIN --

/******************************************************************************
 *  ALS / PS parameter tuning version
 ******************************************************************************/
#define LUX_PER_COUNT_THR_H 3000
#define LUX_PER_COUNT_THR_L 10

//-->[SM32][Sensors][JasonHsing] Add ALS/PS parameter tuning version_2 20170703 END --
...
//<--[SM31][Sensors][JasonHsing] Enable the ALS_DYN_INTT for Light sensor 20161011 BEGIN --
#define ALS_DYN_INTT    1   // ALS Auto INTT
//-->[SM31][Sensors][JasonHsing] Enable the ALS_DYN_INTT for Light sensor 20161011 END --
#define HS_ENABLE       0   // heart beat

#define ALSPS_DBG       1
//<--[SM31][Sensors][JasonHsing] Import the calibration data for P/L-sensor 20161104 BEGIN --
#define PS_LOW_THRESHOLD		1440 // PS low threshold
#define PS_HIGH_THRESHOLD		4000 // PS high threshold
//-->[SM31][Sensors][JasonHsing] Import the calibration data for P/L-sensor 20161104 END --
//<--[SM31][Sensors][JasonHsing] Change Light-sensor golden value 20170301 BEGIN --
//<---[SM32][Sensors][JasonHsing] Change P/L-sensor golden value 20170920 BEGIN --
#define LUX_PER_COUNT	790  //ALS lux per count
//--->[SM32][Sensors][JasonHsing] Change P/L-sensor golden value 20170920 END --
//-->[SM31][Sensors][JasonHsing] Change Light-sensor golden value 20170301 END --
//<--[SM31][Sensors][JasonHsing] Change P-sensor 5cm ratio 20161010 BEGIN --
#define PS_5CM_RATIO         	9/25
//-->[SM31][Sensors][JasonHsing] Change P-sensor 5cm ratio 20161010 END --
/*ALS interrupt table*/
...
int ps_frame_time = 0;
//<--[SM32][Sensors][JasonHsing] Add node to change P/L-sensor parameter by manual 20170712 BEGIN --
static int als_parameter = 2; //0:HW version, 1:DP, 2:SP
static int ps_parameter = 2; //0:HW version, 1:DP, 2:SP
...
//-->[SM32][Sensors][JasonHsing] Add node to change P/L-sensor parameter by manual 20170712 END --

/******************************************************************************
 *  factory setting
 ******************************************************************************/
//<--[SM31][Sensors][JasonHsing] Import the calibration data for P/L-sensor 20161104 BEGIN --
static const char ps_near_file[]="/data/prod/3cm_thresh.dat";
static const char ps_far_file[]="/data/prod/ct_thresh.dat";
static const char ps_cal_file[]="";
static const char als_cal_file[]="/data/prod/lux_per_count.dat";  //als calibration file path
//-->[SM31][Sensors][JasonHsing] Import the calibration data for P/L-sensor 20161104 END --
...
#endif
//<--[SM32][Sensors][JasonHsing] Add node to disable p-sensor 20170629 BEGIN --
static bool ps_enable_flag = true;
//-->[SM32][Sensors][JasonHsing] Add node to disable p-sensor 20170629 END --
...
#endif
#define APS_DBG(fmt, args...)    	    printk(KERN_INFO fmt, ##args)	//Jason.H, change log level, 170518
 ...
 ******************************************************************************/
//<--[SM31][Sensors][JasonHsing] Enable the ALS_DYN_INTT for Light sensor 20161011 BEGIN --
//<---[SM31][Sensors][JasonHsing] Tuning ALS parameter for DP mechanical structure 20161104 BEGIN --
//<----[SM32][Sensors][JasonHsing] Tuning ALS parameter for DP mechanical structure_3 20170705 BEGIN --
#if ALS_DYN_INTT
//Dynamic INTT
...
uint32_t dynamic_intt_max_lux = 32768;
uint32_t dynamic_intt_min_lux = 0;
uint32_t dynamic_intt_min_unit = 1000;
//<--[SM32][Sensors][JasonHsing] Add node to change P/L-sensor parameter by manual 20170712 BEGIN --
static int als_dynamic_intt_intt[] = {EPL_ALS_INTT_512, EPL_ALS_INTT_128};//{EPL_ALS_INTT_1024, EPL_ALS_INTT_32};
static int als_dynamic_intt_value[] = {512, 128}; //match als_dynamic_intt_intt table
...
static int als_dynamic_intt_intt_DP[] = {EPL_ALS_INTT_1024, EPL_ALS_INTT_1024};//{EPL_ALS_INTT_1024, EPL_ALS_INTT_32};
static int als_dynamic_intt_value_DP[] = {1024, 1024}; //match als_dynamic_intt_intt table
static int als_dynamic_intt_gain_DP[] = {EPL_GAIN_MID, EPL_GAIN_LOW};
static int als_dynamic_intt_high_thr_DP[] = {60000, 60000};
static int als_dynamic_intt_low_thr_DP[] = {0, 1200};

static int als_dynamic_intt_intt_SP[] = {EPL_ALS_INTT_512, EPL_ALS_INTT_128};//{EPL_ALS_INTT_1024, EPL_ALS_INTT_32};
static int als_dynamic_intt_value_SP[] = {512, 128}; //match als_dynamic_intt_intt table
static int als_dynamic_intt_gain_SP[] = {EPL_GAIN_MID, EPL_GAIN_LOW};
static int als_dynamic_intt_high_thr_SP[] = {60000, 60000};
static int als_dynamic_intt_low_thr_SP[] = {200, 200};
//-->[SM32][Sensors][JasonHsing] Add node to change P/L-sensor parameter by manual 20170712 END --
static int als_dynamic_intt_intt_num =  sizeof(als_dynamic_intt_value)/sizeof(int);
#endif /*ALS_DYN_INTT*/
//---->[SM32][Sensors][JasonHsing] Tuning ALS parameter for DP mechanical structure_3 20170705 END --
//--->[SM31][Sensors][JasonHsing] Tuning ALS parameter for DP mechanical structure 20161104 END --
//-->[SM31][Sensors][JasonHsing] Enable the ALS_DYN_INTT for Light sensor 20161011 END --
/******************************************************************************
 *  HS_ENABLE
 ******************************************************************************/
...
static void write_global_variable(struct i2c_client *client)
{
    ...
    if(epl_sensor.revno == 0xa188)	//epl update driver, 161014
    	{
...
static bool read_factory_calibration(void)
{
    ...
    {
        fp = filp_open(ps_near_file, O_RDONLY, 0755);
        if (IS_ERR(fp))
        {
            APS_ERR("[%s] NO ps_near_file calibration file\n", __func__);
        ...
        else
        {
            ...
            filp_close(fp, NULL);
            ps_hthr = (buffer[0]<<24)+(buffer[1]<<16)+(buffer[2]<<8)+buffer[3];
            set_fs(fs);
        }
        fp = filp_open(ps_far_file, O_RDONLY, 0755);
        if (IS_ERR(fp))
        {
            APS_ERR("[%s] No ps_far_file calibration file(%d)\n",__func__, (int)IS_ERR(fp));
            epl_sensor.ps.factory.calibration_enable =	false;
        }
        else
        {
            ...
            vfs_read(fp, buffer, sizeof(buffer), &pos);
            filp_close(fp, NULL);
            ps_ct = (buffer[0]<<24)+(buffer[1]<<16)+(buffer[2]<<8)+buffer[3];
            set_fs(fs);
        }
    ...
    {
        fp = filp_open(als_cal_file, O_RDONLY, 0755);
        if (IS_ERR(fp))
        {
            APS_ERR("[%s] NO als_cal_file calibration file\n", __func__);
        ...
        else
        {
            ...
            vfs_read(fp, buffer, sizeof(buffer), &pos);
            filp_close(fp, NULL);
            als_lux_per_count = (buffer[0]<<24)+(buffer[1]<<16)+(buffer[2]<<8)+buffer[3];
//<--[SM32][Sensors][JasonHsing] Widen the ALS calibration range_2 20170703 BEGIN --
            if((als_lux_per_count < LUX_PER_COUNT_THR_L) || (als_lux_per_count > LUX_PER_COUNT_THR_H)){
//-->[SM32][Sensors][JasonHsing] Widen the ALS calibration range_2 20170703 END --
...
//<--[SM32][Sensors][JasonHsing] Add node to change P/L-sensor parameter by manual 20170712 BEGIN --
void epl_sensor_als_parameter(void){

    int idx=0, gain_value=0, intt_value=0, total_value=0;

    switch (als_parameter){
        case HW_VERSION:
                break;
        case DP_PARAMETER:
                memcpy(als_dynamic_intt_intt, als_dynamic_intt_intt_DP, sizeof(als_dynamic_intt_intt_DP));
                memcpy(als_dynamic_intt_value, als_dynamic_intt_value_DP, sizeof(als_dynamic_intt_value_DP));
                memcpy(als_dynamic_intt_gain, als_dynamic_intt_gain_DP, sizeof(als_dynamic_intt_gain_DP));
                memcpy(als_dynamic_intt_high_thr, als_dynamic_intt_high_thr_DP, sizeof(als_dynamic_intt_high_thr_DP));
                memcpy(als_dynamic_intt_low_thr, als_dynamic_intt_low_thr_DP, sizeof(als_dynamic_intt_low_thr_DP));
                break;
        case SP_PARAMETER:
                memcpy(als_dynamic_intt_intt, als_dynamic_intt_intt_SP, sizeof(als_dynamic_intt_intt_SP));
                memcpy(als_dynamic_intt_value, als_dynamic_intt_value_SP, sizeof(als_dynamic_intt_value_SP));
                memcpy(als_dynamic_intt_gain, als_dynamic_intt_gain_SP, sizeof(als_dynamic_intt_gain_SP));
                memcpy(als_dynamic_intt_high_thr, als_dynamic_intt_high_thr_SP, sizeof(als_dynamic_intt_high_thr_SP));
                memcpy(als_dynamic_intt_low_thr, als_dynamic_intt_low_thr_SP, sizeof(als_dynamic_intt_low_thr_SP));
                break;
    }
...
void epl_sensor_ps_parameter(void){
...
//<--[SM31][Sensors][JasonHsing] Reduce P-sensor current by add wait time 20161223 BEGIN --
    epl_sensor.wait = EPL_WAIT_20_MS;
//-->[SM31][Sensors][JasonHsing] Reduce P-sensor current by add wait time 20161223 END --
    ...
    epl_sensor.als.als_std = EPL_ALS_STD; //epl update driver, 161202
...
//<--[SM31][Sensors][JasonHsing] Reduce P-sensor current by add wait time 20161229 BEGIN --
//<---[SM32][Sensors][JasonHsing] Tuning PS parameter for DP mechanical structure_3 20170705 BEGIN --
    epl_sensor.ps.integration_time = EPL_PS_INTT_80;
    epl_sensor.ps.gain = EPL_GAIN_MID;
    epl_sensor.ps.adc = EPL_PSALS_ADC_11;
    epl_sensor.ps.cycle = EPL_CYCLE_8;
//--->[SM32][Sensors][JasonHsing] Tuning PS parameter for DP mechanical structure_3 20170705 END --
//-->[SM31][Sensors][JasonHsing] Reduce P-sensor current by add wait time 20161229 END --
    epl_sensor.ps.ps_rs = EPL_RS_0;
    epl_sensor.ps.ps_intb_nonlos = EPL_T_DIS;
    epl_sensor.ps.persist = EPL_PERIST_1;
    epl_sensor.ps.ps_std = EPL_PS_STD; //epl update driver, 161014
...
static int epl_sensor_get_als_value(struct epl_sensor_priv *obj, u16 als)
{
    ...
    switch(epl_sensor.als.report_type)
    {
		...
		case CMC_BIT_DYN_INT:
            ...
            APS_DBG("[%s]: dynamic_intt_idx=%d, als_dynamic_intt_value=%d, dynamic_intt_gain=%d, als=%d \r\n",
...
static int set_psensor_intr_threshold(uint16_t low_thd, uint16_t high_thd)
{
    ...
    mutex_unlock(&sensor_mutex);
    APS_DBG("%s: low_thd = %d, high_thd = %d \n", __FUNCTION__, low_thd, high_thd);
    return 0;
}
...
//<--[SM32][Sensors][JasonHsing] Add node to disable p-sensor 20170629 BEGIN --
    if(enable == 2){							//enable == 2, enable p-sensor
        ps_enable_flag = true;
        APS_LOG("[%s]: ps_enable_flag = true\r\n", __func__);
        return;
    }
    else if(enable == 3 || ps_enable_flag == false){		//enable == 3, disable p-sensor
        ps_enable_flag = false;
        APS_LOG("[%s]: ps_enable_flag = false\r\n", __func__);
        return;
    }
//-->[SM32][Sensors][JasonHsing] Add node to disable p-sensor 20170629 END --
#if HS_ENABLE
...
int epl_sensor_setup_eint(struct i2c_client *client)
{
	...
	if (obj->irq_node)
	{
		...
		ret = gpio_request(ints[0], "p-sensor");	//fix coverity issues,161103
		if (ret) {
			APS_ERR("request gpio fail, ret=%d\n", ret);
			return -EINVAL;
		}
...
//<--[SM31][Sensors][JasonHsing] P/L-sensor calibration rule for PCBA 20161011 BEGIN --
	obj->hw->ps_threshold_high = high - low;
	obj->hw->ps_threshold_low = ((high - low) * PS_5CM_RATIO);
	epl_sensor.ps.high_threshold = high - low;
	epl_sensor.ps.low_threshold = ((high - low) * PS_5CM_RATIO);
	set_psensor_intr_threshold(epl_sensor.ps.low_threshold, epl_sensor.ps.high_threshold);
//-->[SM31][Sensors][JasonHsing] P/L-sensor calibration rule for PCBA 20161011 END --
    return  count;
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
//<--[SM32][Sensors][JasonHsing] Add node to change P/L-sensor parameter by manual 20170712 BEGIN --
static ssize_t epl_sensor_store_als_parameter(struct device_driver *ddri, const char *buf, size_t count)
{
    int als_par=0;
    struct epl_sensor_priv *obj = epl_sensor_obj;

    APS_FUN();
    sscanf(buf, "%d",&als_par);
    als_parameter = als_par;
    epl_sensor_als_parameter();
    write_global_variable(obj->client);
    epl_sensor_update_mode(obj->client);
    return count;
}
static ssize_t epl_sensor_show_als_parameter(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;

    APS_FUN();
    len += snprintf(buf+len, PAGE_SIZE-len, "als_parameter = %d\r\n", als_parameter);

    return len;
}
/*----------------------------------------------------------------------------*/
static ssize_t epl_sensor_store_ps_parameter(struct device_driver *ddri, const char *buf, size_t count)
{
    int ps_par=0;
    struct epl_sensor_priv *obj = epl_sensor_obj;
    ...
    sscanf(buf, "%d",&ps_par);
    ps_parameter = ps_par;
    epl_sensor_ps_parameter();
    write_global_variable(obj->client);
    epl_sensor_update_mode(obj->client);
    return count;
}

static ssize_t epl_sensor_show_ps_parameter(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;

    APS_FUN();
    len += snprintf(buf+len, PAGE_SIZE-len, "ps_parameter = %d\r\n", ps_parameter);
...
#endif
//<--[SM32][Sensors][JasonHsing] Add node to change P/L-sensor parameter by manual 20170712 BEGIN --
static DRIVER_ATTR(als_parameter,			        S_IWUSR | S_IRUGO, epl_sensor_show_als_parameter, epl_sensor_store_als_parameter                    );
static DRIVER_ATTR(ps_parameter,			        S_IWUSR | S_IRUGO, epl_sensor_show_ps_parameter, epl_sensor_store_ps_parameter                    );
//-->[SM32][Sensors][JasonHsing] Add node to change P/L-sensor parameter by manual 20170712 END --
/*----------------------------------------------------------------------------*/
...
#endif
//<--[SM32][Sensors][JasonHsing] Add node to change P/L-sensor parameter by manual 20170712 BEGIN --
    &driver_attr_als_parameter,
    &driver_attr_ps_parameter,
//-->[SM32][Sensors][JasonHsing] Add node to change P/L-sensor parameter by manual 20170712 END --
};
...
static int als_get_data(int* value, int* status)
{
    ...
    if(epl_sensor.als.saturation == 0x20)	//epl update driver, 161202
    {
...
static int epl_sensor_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    epl_sensor_dumpReg(client); //fix i2c logical issue
...
//<--[SM31][Sensors][JasonHsing] Fix SensorTesttest: BatchAndFlush 20170317 BEGIN --
	//ps_ctl.is_report_input_direct = epl_sensor.ps.polling_mode==0? true:false;
	ps_ctl.is_report_input_direct = false;
//-->[SM31][Sensors][JasonHsing] Fix SensorTesttest: BatchAndFlush 20170317 END --
