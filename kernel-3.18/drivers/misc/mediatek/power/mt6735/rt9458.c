...
static u32 irq_1 = 0, irq_2 = 0, irq_3 = 0;
static int rt9458_irq_gpio = 0, rt9458_irq = 0;
#endif
/*[Arima_8100][bozhi_lin] 20170320 end*/

#define RT9458_DRV_VERSION	"1.0.7_MTK"
...
static int rt9458_reset_chip(struct rt9458_info *info)
{
	...
	/* disable hz */
	ret = rt9458_clr_bit(info, RT9458_REG_CTRL2, RT9458_MASK_HZ_EN);
	if (ret < 0)
		pr_err("%s: disable hz fail\n", __func__);
...
static int rt_charger_enable_otg(struct mtk_charger_info *mchr_info, void *data)
{
	...
	/* Disable HZ before enabling OTG */
	if (enable) {
		ret = rt_charger_enable_hz(mchr_info, &en_hz);
		if (ret < 0) {
...
/*[Arima_8100][bozhi_lin] set ac charging current to 1.5A and enable high voltage battery 20160922 begin*/
	} else if (aicr < 1500000) { /* AICR = 1000mA */
...
/*[Arima_8103][bozhi_lin] fix Coverity issues 20170616 begin*/
/*[Arima_8103][bozhi_lin] fix Coverity issues 20170608 begin*/
	...
	void *data)
{
	...
	ret = rt9458_get_charging_status(info, &chg_status);
	if (ret < 0)
		pr_info("%s(%d): rt9458_get_charging_status failed\n", __func__, __LINE__);
	if (chg_status == RT9458_CHG_STATUS_FAULT) {
		for (i = 0; i < ARRAY_SIZE(rt9458_reg_addr); i++) {
			...
			if (ret < 0)
				pr_info("%s(%d): rt9458_i2c_read_byte failed\n", __func__, __LINE__);
		}
	}

	ret = rt_charger_get_ichg(&info->mchr_info, &ichg);
	if (ret < 0)
		pr_info("%s(%d): rt_charger_get_ichg failed\n", __func__, __LINE__);

	ret = rt9458_get_mivr(info, &mivr);
	if (ret < 0)
		pr_info("%s(%d): rt9458_get_mivr failed\n", __func__, __LINE__);

	ret = rt_charger_get_aicr(&info->mchr_info, &aicr);
	if (ret < 0)
		pr_info("%s(%d): rt_charger_get_aicr failed\n", __func__, __LINE__);

	ret = rt9458_get_ieoc(info, &ieoc);
	if (ret < 0)
		pr_info("%s(%d): rt9458_get_ieoc failed\n", __func__, __LINE__);

	ret = rt9458_is_charging_enable(info, &chg_enable);
	if (ret < 0)
		pr_info("%s(%d): rt9458_is_charging_enable failed\n", __func__, __LINE__);
...
/*[Arima_8103][bozhi_lin] 20170608 end*/
/*[Arima_8103][bozhi_lin] 20170616 end*/
...
/*[Arima_8103][bozhi_lin] rt9458 disable AICR 20170704 begin*/
#if 0
...
#endif
/*[Arima_8103][bozhi_lin] 20170704 end*/
...
static int rt_charger_enable_te_shutdown(struct mtk_charger_info *mchr_info, void *data)
{
	int ret = 0;
	struct rt9458_info *info = (struct rt9458_info *)mchr_info;

	u8 enable = *((u8 *)data);
	ret = rt9458_enable_te_shutdown(info, enable);
...
/*[Arima_8100][bozhi_lin] RID003699 OVP warning message to end-user 20170320 begin*/
static int rt9458_get_vinovpi(struct rt9458_info *info, u32 *vinovpi)
{
	int ret = 0;

	ret = rt9458_i2c_read_byte(info, RT9458_REG_IRQ1);
	if (ret < 0)
		return ret;

	//*vinovpi = (ret & RT9458_MASK_VINOVPI) >> RT9458_SHIFT_VINOVPI;
	*vinovpi = info->vinovpi_flag;

	return ret;
}

static int rt_charger_get_vinovpi(struct mtk_charger_info *mchr_info, void *data)
{
	int ret = 0;
	u32 vinovpi = 0;
	struct rt9458_info *info = (struct rt9458_info *)mchr_info;

	ret = rt9458_get_vinovpi(info, &vinovpi);
...
static int rt_charger_get_is_charging(struct mtk_charger_info *mchr_info, void *data)
{
	int ret = 0;
	struct rt9458_info *info = (struct rt9458_info *)mchr_info;
	enum rt9458_charging_status chg_status = RT9458_CHG_STATUS_READY;
...
static int rt_charger_dump_register_irq(struct mtk_charger_info *mchr_info,
	void *data)
{
	int ret = 0;
	struct rt9458_info *info = (struct rt9458_info *)mchr_info;
	...
	ret = rt9458_i2c_read_byte(info, RT9458_REG_IRQ2);
	if (ret < 0)
		return ret;
	irq_2 = ret;

	ret = rt9458_i2c_read_byte(info, RT9458_REG_IRQ3);
	if (ret < 0)
		return ret;
	irq_3 = ret;

	info->vinovpi_flag = (irq_1 & RT9458_MASK_VINOVPI) >> RT9458_SHIFT_VINOVPI;
	pr_info("[B]%s(%d): irq_1=0x%02x, irq_2=0x%02x, irq_3=0x%02x, vinovpi_flag=%d\n", __func__, __LINE__, irq_1, irq_2, irq_3, info->vinovpi_flag);
...
static int rt9458_event_handler(void *unused)
{
	...
	do
	{
		gpio_val = __gpio_get_value(rt9458_irq_gpio);
		//pr_info("[B]%s(%d): rt9458_irq_gpio=%d\n", __func__, __LINE__, gpio_val);
		...
		battery_charging_control(CHARGING_CMD_DUMP_REGISTER_IRQ, NULL);

		enable_irq(rt9458_irq);
		gpio_val = __gpio_get_value(rt9458_irq_gpio);
		//pr_info("[B]%s(%d): rt9458_irq_gpio=%d\n", __func__, __LINE__, gpio_val);
...
static int rt9458_irq_registration(void)
{
	...
	if (node) {
		rt9458_irq_gpio = of_get_named_gpio(node, "rt9458,irq_gpio", 0);
		pr_info("[B]%s(%d): rt9458_irq_gpio = %d\n", __func__, __LINE__, rt9458_irq_gpio);
	
		of_property_read_u32_array(node,"debounce", ints, ARRAY_SIZE(ints));
		gpio_set_debounce(ints[0], ints[1]);

		rt9458_irq = irq_of_parse_and_map(node, 0);
		pr_info("[B]%s(%d): rt9458_irq = %d\n", __func__, __LINE__, rt9458_irq);

		ret = request_irq(rt9458_irq, rt9458_eint_interrupt_handler,
...
/*[Arima_8100][bozhi_lin] charging maintenance implement 20161102 begin*/
#if defined(CHARGING_MAINTAIN)
	[CHARGING_CMD_ENABLE_TE_SHUTDOWN] = rt_charger_enable_te_shutdown,
#endif
/*[Arima_8100][bozhi_lin] 20161102 end*/
/*[Arima_8100][bozhi_lin] RID003699 OVP warning message to end-user 20170320 begin*/
	[CHARGING_CMD_GET_VINOVPI] = rt_charger_get_vinovpi,
	[CHARGING_CMD_GET_IS_CHARGING] = rt_charger_get_is_charging,
	[CHARGING_CMD_DUMP_REGISTER_IRQ] = rt_charger_dump_register_irq,
/*[Arima_8100][bozhi_lin] 20170320 end*/
 ...
 * Revision Note
 * 1.0.8
 * (1) Disable HZ before reset chip
 *
 * 1.0.7
 * (1) Disable HZ before enabling OTG
 *
 * 1.0.6
