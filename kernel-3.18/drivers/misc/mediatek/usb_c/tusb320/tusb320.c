...
enum typec_current_mode tusb320_current_mode_detect(void);
...
char *tusb_show_vendor_id = NULL;
module_param(tusb_show_vendor_id, charp, 0444);
...
/*[Arima_8103][bozhi_lin] fix off-charging will cause ram dump wiith pd charger 20170628 begin*/
	if (g_tusb320_dev == NULL) {
		pr_info("[B]%s(%d): g_tusb320_dev not ready, return TYPEC_CURRENT_MODE_DEFAULT\n", __func__, __LINE__);
		return current_mode;
	}
/*[Arima_8103][bozhi_lin] 20170628 end*/
...
/*[Arima_8103][bozhi_lin] add tusb320 type-c show vendor id 20170517 begin*/
	int i = 0; 
	for (i = 0 ; i < 8 ; i++) {
		ret = tusb320_read_reg(TUSB320_REG_DEVICE_ID_END - i, &reg_val);
		if (ret < 0) {
			pr_err("%s: read REG_DEVICE_ID error\n", __func__);
			return ret;
		}
		di->device_id[i] = reg_val;
		//pr_info("[B]%s(%d): di->device_id[%d]=0x%x, reg_val=0x%x\n", __func__, __LINE__, i, di->device_id[i], reg_val);
	}
/*[Arima_8103][bozhi_lin] 20170517 end*/
	return 0;
}
...
//		}
	...
	if (!timeout) {
		...
		timeout = wait_for_completion_timeout(&di->reverse_completion,
					    ...
					    (DUAL_ROLE_SET_MODE_WAIT_MS));
		if (!timeout)
			pr_err("%s(%d): reverse failed, timeout=%d\n", __func__, __LINE__, timeout);
		/*[Arima_8103][bozhi_lin] 20170608 end*/
		ret = -EIO;
	}
	pr_err("%s: end ret = %d\n", __func__, ret);
...
static int usbc_pinctrl_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(typec->pinctrl)) {
		pr_info("[B]%s(%d): Cannot find usb pinctrl!\n", __func__, __LINE__);
	} else {
		typec->pin_cfg = kzalloc(sizeof(struct usbc_pin_ctrl), GFP_KERNEL);

		pr_info("[B]%s(%d): pinctrl=%p\n", __func__, __LINE__, typec->pinctrl);

		typec->pin_cfg->tusb320_en_low = pinctrl_lookup_state(typec->pinctrl, "tusb320_en_low");
		if (IS_ERR(typec->pin_cfg->tusb320_en_low))
			pr_info("[B]%s(%d): Can *NOT* find tusb320_en_low\n", __func__, __LINE__);
		else
			pr_info("[B]%s(%d): Find tusb320_en_low\n", __func__, __LINE__);

		typec->pin_cfg->tusb320_en_high = pinctrl_lookup_state(typec->pinctrl, "tusb320_en_high");
		if (IS_ERR(typec->pin_cfg->tusb320_en_high))
			pr_info("[B]%s(%d): Can *NOT* find tusb320_en_high\n", __func__, __LINE__);
		else
			pr_info("[B]%s(%d): Find tusb320_en_high\n", __func__, __LINE__);

		pr_info("[B]%s(%d): Finish parsing pinctrl\n", __func__, __LINE__);
	}
			 ...
			 const struct i2c_device_id *id)
{
...
#if 1
	node = of_find_node_by_name(NULL, "tusb320_typec");
#else
	...
	if (IS_ENABLED(CONFIG_DUAL_ROLE_USB_INTF)) {
		desc = devm_kzalloc(cdev, sizeof(struct dual_role_phy_desc), GFP_KERNEL);
		if (!desc) {
...
/*[Arima_8103][bozhi_lin] add tusb320 type-c show vendor id 20170517 begin*/
	{
		char buf[20]={0};
		sprintf(buf, "TypeC_%s", &di->device_id[1]);
		if (tusb_show_vendor_id == NULL) {
			tusb_show_vendor_id = kmalloc(strlen(buf) + 1, GFP_KERNEL);
		}
		if (tusb_show_vendor_id != NULL) {
			strcpy(tusb_show_vendor_id, buf);
		}
	}
/*[Arima_8103][bozhi_lin] 20170517 end*/
...
/*[Arima_8103][bozhi_lin] TUSB320 TypeC set EN to low at suspend for power consumption 20170602 begin*/
static int tusb320_suspend(struct device *dev)
{
	pr_info("[B]%s(%d): set EN to Low\n", __func__, __LINE__);
	pinctrl_select_state(g_exttypec->pinctrl, g_exttypec->pin_cfg->tusb320_en_low);
	return 0;
}

static int tusb320_resume(struct device *dev)
{
	pr_info("[B]%s(%d): set EN to High\n", __func__, __LINE__);
	pinctrl_select_state(g_exttypec->pinctrl, g_exttypec->pin_cfg->tusb320_en_high);
	return 0;
}

static const struct dev_pm_ops tusb320_pm = {
	.suspend = tusb320_suspend,
	.resume = tusb320_resume
};
/*[Arima_8103][bozhi_lin] 20170602 end*/
...
static __init int tusb320_i2c_init(void)
{
	...
	if (ret) {
		...
		if (!platform_driver_register(&usbc_pinctrl_driver)) {
			pr_info("[B]%s(%d): register usbc pinctrl succeed!!\n", __func__, __LINE__);
			pinctrl_select_state(g_exttypec->pinctrl, g_exttypec->pin_cfg->tusb320_en_high);
			pr_info("[B]%s(%d): set EN to High\n", __func__, __LINE__);
		} else {
			pr_err("[B]%s(%d): register usbc pinctrl fail!!\n", __func__, __LINE__);
		}
	}
	pr_info("%s: end\n", __func__);
	return ret;
