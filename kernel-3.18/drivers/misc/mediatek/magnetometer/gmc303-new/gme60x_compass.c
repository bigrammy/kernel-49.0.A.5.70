...
static long AKI2C_RxData(char *rxData, int length)
{
	...
	if ((rxData == NULL) || (length < 1))
		return -EINVAL;
#if DEBUG
	addr = rxData[0];	//170612, JasonH, fix coverity
#endif
	mutex_lock(&gme60x_i2c_mutex);
...
static long gme60x_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
  ...
  //	MAG_ERR("gme60x cmd:0x%x\n", cmd);	
	...
	case ECS_IOCTL_GET_DELAY_60X:
		...
		delay[0] = (int64_t)gmad_delay * 1000000;	//170612, JasonH, fix coverity
		delay[1] = (int64_t)gmad_delay * 1000000;
		delay[2] = (int64_t)gmad_delay * 1000000;
...
#endif
	data->client = client;
	data->client->addr = 0x0D;	//forced change i2c slave addr
	printk("[%s] kernel: auto probe address 0x0D\n", __func__);
