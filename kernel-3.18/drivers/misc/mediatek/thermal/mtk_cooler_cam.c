...
//<--[SM32][RID005711][JasonHsing] Video rec stop feature for thermal mitigation 20170714 BEGIN --
static int ov_cri_count = 10;
static int rec_cri_happened = 0;
static int rec_cri_count = 60;
//-->[SM32][RID005711][JasonHsing] Video rec stop feature for thermal mitigation 20170714 END --
...
//<--[SM32][RID005711][JasonHsing] Video rec stop feature for thermal mitigation 20170714 BEGIN --
	if (1 == state){
		_cl_cam = 1;
		rec_cri_count++;
		if((strcmp(cdev->type, "mtk-cl-cam00") == 0) && (rec_cri_count >= 60)){
			cam_send_signal(18);
			rec_cri_count = 0;
			rec_cri_happened = 1;
		}
...
//<--[SM32][RID005711][JasonHsing] Video rec stop feature for thermal mitigation 20170714 BEGIN --
		if (i == MAX_NUM_INSTANCE_MTK_COOLER_CAM){
			_cl_cam = 0;
			if((strcmp(cdev->type, "mtk-cl-cam00") == 0) && (1 == rec_cri_happened)){
				cam_send_signal(19);
				rec_cri_count = 60;
				rec_cri_happened = 0;
			}
