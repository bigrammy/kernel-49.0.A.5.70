...
/*[Arima_8103][bozhi_lin] add smmoth ui mechanism to meet user feeling 20171110 begin*/
static kal_bool SMOOTH_UI_SOC2_enable = KAL_FALSE;
static signed int SMOOTH_UI_SOC2 = -1;
/*[Arima_8103][bozhi_lin] 20171110 end*/
...
int __batt_meter_init_cust_data_from_dt(void)
{
	...
	while (!of_property_read_u32_index(np, "batt_temperature_table", idx, &addr)) {
		...
		if (!of_property_read_u32_index(np, "batt_temperature_table", idx, &val))
			bm_debug("batt_temperature_table: addr: %d, val: %d\n", addr, val);
//<20170505-YungwenPeng, Fixed : error: request for member 'BatteryTemp' in something not a structure or union
		Batt_Temperature_Table[idx / 2]->BatteryTemp = addr;
		Batt_Temperature_Table[idx / 2]->TemperatureR = val;
//>20170505-YungwenPeng
		idx++;
		if (idx >= num * 2)
			break;
	}
...
#if defined(BATTERY_DTS_SUPPORT) && defined(CONFIG_OF)
/*[Arima_8103][bozhi_lin] charging parameter only check header, not check dtsi 20170511 begin*/
#if 0
...
/*[Arima_8103][bozhi_lin] add smmoth ui mechanism to meet user feeling 20171128 begin*/
/*[Arima_8103][bozhi_lin] add smmoth ui mechanism to meet user feeling 20171115 begin*/
/*[Arima_8103][bozhi_lin] add smmoth ui mechanism to meet user feeling 20171110 begin*/
void battery_meter_smooth_uisoc2_ex(signed int UI_SOC)
{
	static signed int start_SOC = -1;
	static signed int start_UI_SOC2 = -1;
	static signed int count = 6;

	//battery_log(BAT_LOG_CRTI,"[kernel] %d: SOC %3d:%3d:%3d\n", __LINE__, start_SOC, start_UI_SOC2, SMOOTH_UI_SOC2);

	if ((SMOOTH_UI_SOC2 == -1) && (SMOOTH_UI_SOC2_enable == KAL_FALSE)) {
		if ((BMT_status.SOC < UI_SOC) && ((UI_SOC - BMT_status.SOC) >= 5)) {
			SMOOTH_UI_SOC2_enable = KAL_TRUE;
			start_SOC = BMT_status.SOC;
			if (BMT_status.UI_SOC > UI_SOC) {
				start_UI_SOC2 = BMT_status.UI_SOC;
				SMOOTH_UI_SOC2 = BMT_status.UI_SOC;
			} else {
				start_UI_SOC2 = UI_SOC;
				SMOOTH_UI_SOC2 = UI_SOC;
			}
//		} else if ((BMT_status.SOC > UI_SOC) && ((BMT_status.SOC - UI_SOC) >= 10)) {
//			battery_meter_smooth_uisoc2_ex2(UI_SOC);
		} else {
			return;
		}
	}
	...
	if (gFG_coulomb_is_charging == 1) {
		if (SMOOTH_UI_SOC2_enable == KAL_TRUE) {
			if (SMOOTH_UI_SOC2 == UI_SOC) { //once reach UI_SOC, disable
				SMOOTH_UI_SOC2_enable = KAL_FALSE;
				start_SOC = -1;
				start_UI_SOC2 = -1;
				count = 0;
			} else if (SMOOTH_UI_SOC2 > UI_SOC) { // wait

			} else if (SMOOTH_UI_SOC2 < UI_SOC) { // every 6 times to increase 1
				count++;
				if (count >= 6) {
					SMOOTH_UI_SOC2 ++;
					count = 0;
		...
		if (SMOOTH_UI_SOC2 > start_SOC) { // every 6 times to decrease 1
			count++;
			if (count >= 6) {
				SMOOTH_UI_SOC2--;
				count = 0;
			...
			if (SMOOTH_UI_SOC2 == UI_SOC) { //once reach UI_SOC, disable
				...
				start_UI_SOC2 = -1;
				count = 0;
			} else if (SMOOTH_UI_SOC2 > UI_SOC) { // every 6 times to decrease 1
				count++;
				if (count >= 6) {
					SMOOTH_UI_SOC2 --;
					count = 0;
	...
	battery_log(BAT_LOG_CRTI,"[kernel] %d: SOC %3d:%3d:%3d:%3d:%3d:%3d\n", __LINE__, BMT_status.SOC, BMT_status.UI_SOC, BMT_status.UI_SOC2, start_SOC, start_UI_SOC2, SMOOTH_UI_SOC2);
}
/*[Arima_8103][bozhi_lin] 20171110 end*/
/*[Arima_8103][bozhi_lin] 20171115 end*/
/*[Arima_8103][bozhi_lin] 20171128 end*/
...
/*[Arima_8103][bozhi_lin] add battery_id to show battery vendor 20170703 begin*/
/* ------------------------------------------------------------------------------------------- */

static ssize_t show_FG_battery_id(struct device *dev, struct device_attribute *attr, char *buf)
{
	bm_debug("[FG] show g_fg_battery_id : %d\n", g_fg_battery_id);
	if (g_fg_battery_id == 0) {
		return sprintf(buf, "TDK_2nd");
	} else if (g_fg_battery_id == 1) {
		return sprintf(buf, "SEND_Main");
	}
	return sprintf(buf, "NULL");
}

static ssize_t store_FG_battery_id(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	return size;
}

static DEVICE_ATTR(FG_battery_id, 0664, show_FG_battery_id, store_FG_battery_id);
...
/*[Arima_8103][bozhi_lin] add battery_id to show battery vendor 20170703 begin*/
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_FG_battery_id);
/*[Arima_8103][bozhi_lin] 20170703 end*/
...
/*[Arima_8103][bozhi_lin] add reset file node for battery capacity reset 20170713 begin*/
			if ((BMT_status.is_rtc_reset) == KAL_TRUE) {
				rtcvalue = 0;
			}
...
#if 1
			if (SMOOTH_UI_SOC2_enable == KAL_TRUE) {
				if ((rtcvalue <= 1) && (BMT_status.bat_vol < 3400)) {
					set_rtc_spare_fg_value(rtcvalue);
					bm_notice("[fg_res] set rtc = %d\n", rtcvalue);
				} else {
					set_rtc_spare_fg_value(SMOOTH_UI_SOC2);
					bm_notice("[fg_res] set rtc = %d\n", SMOOTH_UI_SOC2);
				}
			} else {
				set_rtc_spare_fg_value(rtcvalue);
...
/*[Arima_8103][bozhi_lin] add smmoth ui mechanism to meet user feeling 20171110 begin*/
			//battery_log(BAT_LOG_CRTI, "[kernel]%d: SOC %3d:%3d:%3d:%3d:%3d\n", __LINE__, BMT_status.SOC, BMT_status.UI_SOC, BMT_status.UI_SOC2, UI_SOC, SMOOTH_UI_SOC2);
			if (UI_SOC > 0) {
				battery_meter_smooth_uisoc2_ex(UI_SOC);
			}
/*[Arima_8103][bozhi_lin] 20171110 end*/
#ifdef USING_SMOOTH_UI_SOC2
			temp_UI_SOC2 = UI_SOC;
...
#if 1
			if (SMOOTH_UI_SOC2_enable == KAL_TRUE) {
				BMT_status.UI_SOC2 = SMOOTH_UI_SOC2;
			} else {
